...
public class Network_Manager : MonoBehaviour
{
	...
	private List<Vector3> vectors;
        public static Network_Manager _Instance;
         public static Network_Manager Instance
    {
        get
        {
            if (!_Instance)
            {
                _Instance = new GameObject().AddComponent<Network_Manager>();
        ...
        int numNetworkManagers = 0;
static Boolean notFirstTime;


    void Awake()
    {
        
         numNetworkManagers = FindObjectsOfType<Scene_Manager>().Length;
        Debug.Log("numNetworkManagers " + numNetworkManagers);
        GameObject[] objs = GameObject.FindGameObjectsWithTag("Network Tag");
        Debug.Log("OBJS LENGTH PRE NETWORK: " + objs.Length);
        foreach(GameObject obj in objs){
            Debug.Log("THE GAME OBJECT IS (IN NETWORK MANAGER): " + this);

        }
        if (objs.Length > 1)
        {
            Destroy(this);
            Debug.Log("OBJS DESTROYED: " + this);

        }
        else{
            DontDestroyOnLoad(this);
        }
        Debug.Log("OBJS LENGTH POST NETWORK" + objs.Length);
        foreach(GameObject obj in objs){
            Debug.Log("THE GAME OBJECT IS POST (IN NETWORK MANAGER): " + this.gameObject);
        }
numNetworkManagers = FindObjectsOfType<Scene_Manager>().Length;
        Debug.Log("numNetworkManagers " + numNetworkManagers);
        isConnected = true;
...
string json = JsonConvert.SerializeObject(newestReport,  new JsonSerializerSettings()
        { 
            ReferenceLoopHandling = ReferenceLoopHandling.Ignore
        });
//string json1 = JsonConvert.SerializeObject(newestReport, Formatting.Indented, new KeysJsonConverter(typeof(altReport)));
...
//return o.ToString();
    ...
    string finalTestString = JsonConvert.SerializeObject(finalTest/*report*/,  new JsonSerializerSettings()
                    ...
                    TestCasesCollection.InsertOne(document2);
...
//}
   ...
   public string /*BsonDocument*/ setupDrawingObject(){
       ...
       string stringToDeserialize="";
        ...
        massiveStringFromReports = massiveStringFromReports.Replace("} ], 'paintColors'", "], 'paintColors'");
        massiveStringFromReports = massiveStringFromReports.Replace("} ], 'endOfDocument'", "], 'endOfDocument'");
        massiveStringFromReports = massiveStringFromReports.Replace("} ], PlaneCenters :", "], 'PlaneCenters' :");

        massiveStringFromReports = massiveStringFromReports.Replace("} } ], 'colorList'", "} ], 'colorList'");
        massiveStringFromReports = massiveStringFromReports.Replace("} } ], headings", "} ], 'headings'");
        massiveStringFromReports = massiveStringFromReports.Replace("} ], AxialDistances", "], 'AxialDistances'");
        massiveStringFromReports = massiveStringFromReports.Replace("} ], PlaneCentersToArt", "], 'PlaneCentersToArt'");
       // massiveStringFromReports = massiveStringFromReports.Replace("} ], 'PlanceCenters'", "], 'PlaneCenters'");
        massiveStringFromReports = massiveStringFromReports.Replace("} } ], CameraRotationsY", "} ], 'CameraRotationsY'");
